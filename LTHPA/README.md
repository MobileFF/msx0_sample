...
GPSが測位を開始するまでには、１分程度かかる場合もあります。
移動中の位置情報を記録する場合は、プログラム開始後に位置情報の測位が開始されてから移動を始める必要があるようです。

以下の画面ではまだGPSが測位を開始していませんが、測位が開始されると北緯、南緯を示すN/Sの表示と緯度、東経、西経を示すE/Wの表示と経度が表示、記録されていきます。
